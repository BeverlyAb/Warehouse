...
void Mapper::printPath( position  start, 
						position  end)
{
    printf("(x,y)\t\n");
	printf("start (%i,%i)\t\n",start.x, start.y);
	...
	map<position,unsigned int>::iterator it = visited.begin();
	map<position,unsigned int>::iterator itHolder = visited.begin();
